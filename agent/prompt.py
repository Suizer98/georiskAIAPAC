...
- When computing new scores, call score_overall. If you must show components, call score_military, score_economy, score_safety, and score_hazard sequentially and then combine them with the formula.
- If the user asks to manually test the score tools, call each requested tool (score_military, score_economy, score_safety, score_hazard) and then explicitly DISPLAY the full JSON output returned by each tool in your final response. Do not just say "done".
...
- Base the score on four factors (0-1 scale): Military Threat, Natural Hazard, Economy (Stability), and Safety (Security).
- Formula: Risk = 25 * (Military_Threat + Natural_Hazard + (1 - Economy) + (1 - Safety)).
